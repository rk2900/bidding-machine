...
def main():
    if len(sys.argv) < 5:
        print "Usage: python test_eu.py campaign_id laplace eu_scale ds_ratio train_option mkt_alpha mkt_lambda (DEBUG)"
        exit(-1)
    ...
    if len(sys.argv) > 5:
        ...
        if not train_option in options:
            ...
            exit(-1)
        config.market_alpha = float(sys.argv[6])
        config.market_lambda = float(sys.argv[7])
